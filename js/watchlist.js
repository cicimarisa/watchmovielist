        ...
        const moviesHtml = watchlistsFromLocal.map(item => {
            ...
            return `<div class="movie-container d-flex">
                ...
                <div class="${item.backdrop_path ? 'w-80' : 'w-100'}">
                    ...
                    <p class="strong"><span class="${genresOfMovie.length > 0 ? 'genre' : ''}">${genresOfMovie.join(" ")}</span><span class="action-whitelist"><button class="remove" id="${item.id}">-</button> Remove</span></p>
                    <p>${item.overview}</p>
                </div>
            </div>`
        }).join("")
